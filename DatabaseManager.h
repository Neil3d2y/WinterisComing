...
#ifndef TINYSQL_DATABASEMANAGER_H
#define TINYSQL_DATABASEMANAGER_H
...
private:
    ...
    void TableScan(string& table_name, vector<string>& select_list)
    {
            ...
            if (CP.HasExpression())
            {
                ...
                if (natural_join_list.size() > 0)
                {
                    ...
                    else if (IsSort && IsDistinct)
                    {
                        ...
                        vector<string> tmp_attr_names = schema_nj.getFieldNames();
                        vector<enum FIELD_TYPE> tmp_attr_types = schema_nj.getFieldTypes();
                        if (!CreateTable(tmp_table_name, tmp_attr_names, tmp_attr_types))
                            return false;
                        Relation* relation_tmp = schemaMng->getRelation(tmp_table_name);
                        PreSortTwoPass(relation_nj, select_list);
...
#endif //TINYSQL_DATABASEMANAGER_H
